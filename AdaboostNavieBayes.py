...
def trainingNaiveBayes(trainMarkedWords, trainCategory):
    """
    训练数据集中获取词袋模型中词汇的spamicity：P（Wi|S）
    :param trainMarkedWords: 按照词袋模型标记的数据，二维数组
    :param trainCategory:
    :return:
    """
    ...
    for i in range(0, numTrainDoc):
        if trainCategory[i] == 1:  # 如果是垃圾SMS或邮件
            ...
            spamWordsNum += sum(trainMarkedWords[i])  # 统计Spam中词袋模型中词汇出现的总次数
        else:
...
def getTrainedModelInfo():
    ...
    """
    # 加载训练获取的词袋模型信息
    vocabularyList = getVocabularyList('./train_model/vocabularyList.txt')
